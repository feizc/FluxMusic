...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # text = self.data[index]['label']# self.get_sample_text_caption(datum, mix_datum, label_vector)

        if random.random() < self.uncond_pro: 
            txt = ""
        else:
            txt = self.data[index]['label']

        # Get text for t5
        if random.random() < self.uncond_pro_t5:
            txt_t5 = ""
        else:
            txt_t5 = self.data[index]['label']

        # Use encoder to embed tokens online
        text, text_embedding, text_embedding_mask = self.get_text_info_with_encoder(txt)
        text_t5, text_embedding_t5, text_embedding_mask_t5 = self.get_text_info_with_encoder_t5(txt_t5)


        data = {
            "text_embedding": text_embedding.clone().detach(),  # list
            "text_embedding_mask": text_embedding_mask.clone().detach(),
            "text_embedding_t5": text_embedding_t5.clone().detach(),
            "text_embedding_mask_t5": text_embedding_mask_t5.clone().detach(),
